...
const getCartesianSeries = (series) => {
    if (!Array.isArray(series)) {
        return [];
    }

    return series.filter((item) => item && Array.isArray(item.data));
};

const getDonutSeries = (series) => {
    if (!Array.isArray(series)) {
        return [];
    }

    return series.filter((item) => typeof item === "number");
};

const AudiencesCharts = ({ dataColors, series }) => {
    var chartAudienceColumnChartsColors = getChartColorsArray(dataColors);
    const safeSeries = getCartesianSeries(series);
    var options = {
        chart: {
            type: 'bar',
            height: 309,
...
const UsersByDeviceCharts = ({ dataColors, series }) => {
    ...
    const safeSeries = getDonutSeries(series);
    const options = {
